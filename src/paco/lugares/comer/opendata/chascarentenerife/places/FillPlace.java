package paco.lugares.comer.opendata.chascarentenerife.places;
...
public class FillPlace extends AsyncTask<String, Void, ArrayList<String>>
{
	...
	protected ArrayList<String> doInBackground(String... args) {
		...
		try{
			...
			String contiene = args[0].toLowerCase();
			if (!contiene.contains("tenerife"))
				direccion+=" Tenerife";
			if (!contiene.contains("spain"))
				direccion+=" Spain";
			URL googlePlaces = new URL("https://maps.googleapis.com/maps/api/place/autocomplete/json?input="+ URLEncoder.encode(direccion, "UTF-8") +"&types=geocode&language=en&sensor=true&key="+context.getResources().getString(R.string.key_api_place));
